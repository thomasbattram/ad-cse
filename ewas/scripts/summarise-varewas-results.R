...
## args tests
# res_files <- paste0("results/ewas/varewas-res-", c("cc", "no-cc", "no-covs"), ".tsv")
# cse_summ_file <- "results/summary-of-results.RData"
# plotfile <- "results/var-ewas-man-qq.png"
# summ_outfile <- "results/var-ewas-summary-of-results.RData"

res_files <- unlist(str_split(res_files, " "))

## data
mods <- gsub("varewas-res-|.tsv", "", basename(res_files))
