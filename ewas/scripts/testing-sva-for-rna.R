...
library(eigenR2) # for estimating number of SVs to use
...
## NOTES:
# This code assumes 5 SVs
# It assumes you have:
	# a data.frame of SVs called "svs" and the columns are IID, sv1, sv2, sv3, sv4, sv5
	# and a matrix of transcriptome data called "rna_data"
# It might take a while to run 

# install.packages("devtools")
devtools::install_github("StoreyLab/eigenR2")
library(eigenR2)
library(tidyverse)
...
sv_file <- ""
load(rna_data_file)
svs <- read_tsv(sv_file)

rna_data <- rna_data[, colnames(rna_data) %in% svs$IID]
if (!all(colnames(rna_data) == svs$IID)) {
	stop("Please make sure the columns of rna_data are in the same order as the svs rows")
}

nsvs <- 5
r2_vals <- lapply(1:nsvs, function(i) {
	print(i)
	sv_nums <- seq(1, i)
	sv_nams <- paste0("sv", sv_nums)
	mi_form <- as.formula(paste0("~1 + ", paste0(sv_nams, collapse = " + ")))
	mi <- model.matrix(mi_form, svs)
	ri <- eigenR2(dat = t(rna_data), model = mi, null.model = NULL)$eigenR2
	return(ri)
})
r2_vals_vec <- unlist(r2_vals)

r2_vals_final <- lapply(nsvs:1, function(i) {
	r2_i <- r2_vals_vec[i]
	r2_o <- r2_vals_vec[1:(i-1)]
	r2_out <- r2_i - r2_o
	return(r2_out)
})
names(r2_vals_final) <- paste0("sv", 1:5)
